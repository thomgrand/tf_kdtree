...
public:
    ...
    CUDA_HOSTDEV static inline T_ind compParentInd(const T_ind lin_ind) { return (lin_ind - 1)/2; }

    template <typename T_ind>
    CUDA_HOSTDEV static inline T_ind compLeftChildInd(const T_ind lin_ind) { return lin_ind * 2 + 1; }

    template <typename T_ind>
    CUDA_HOSTDEV static inline T_ind compRightChildInd(const T_ind lin_ind) { return lin_ind * 2 + 2; }
    ...
    CUDA_HOSTDEV static inline T_ind compLeftLeafInd(const T_ind lin_ind, const T_ind2 nr_partitions) { return compLeftChildInd(lin_ind) - nr_partitions; }

    template <typename T_ind, typename T_ind2>
    CUDA_HOSTDEV static inline T_ind compRightLeafInd(const T_ind lin_ind, const T_ind2 nr_partitions) { return compLeftLeafInd(lin_ind, nr_partitions) + 1; }

    template <typename T_ind>
    CUDA_HOSTDEV static inline T_ind compLeftLeafInd(const T_ind lin_ind) { return compLeftLeafInd(lin_ind, compTotalNrNodes(compLevel(lin_ind))); }

    template <typename T_ind>
    CUDA_HOSTDEV static inline T_ind compRightLeafInd(const T_ind lin_ind) { return compLeftLeafInd(lin_ind) + 1; }
    ...
    { 
        ...
        //const unsigned char bit_idx  = current_lin_ind % 4; //Not used
